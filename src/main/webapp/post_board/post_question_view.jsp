...
<dl style="width: 1240px; margin-right: auto; margin-left: auto;">
	<dd>
		<table border="0" class="post_table">
			...
			<tr>
				<th style="text-align: center;" width="100px" height="50px">순번</th>
			...
			<tr>
				<td align="center" style="height: 80px;" colspan="5">
					<a href="post.jsp?type=<%= type %>&page=<%= curpage %>&p_no=<%= p_no %>" class="btn mint">글목록</a>
					<%if(login_level == null)
					{ %>
						<input type="hidden" name="hidden">
					<%}else if((login_level.equals("A") || u_no.equals(login_no) )){ %>
					<a href="post_question_modify.jsp?type=<%= type %>&page=<%= curpage %>&p_no=<%= p_no %>" class="btn mint">글수정</a>
